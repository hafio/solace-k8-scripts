...
## Corpus Check
- 85 files · ~184,122 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1760 nodes · 5322 edges · 73 communities (65 shown, 8 thin omitted)
...
### Community 8 - "Config"
Cohesion: 0.09
Nodes (32): Admin, Container, ContainerSecurity, DockerConfig, DomainCerts, Image, Network, Node (+24 more)
...
Cohesion: 0.16
Nodes (27): strings.Builder, PodAffinityTerm, boolStr(), BrokerCR(), containerSecretSpecs(), cut(), groupKey(), itoa() (+19 more)
...
### Community 34 - "allowcommand_test.go"
Cohesion: 0.33
...
Cohesion: 0.17
Nodes (27): emitCtrArtifact(), NodeIdentity, Compose(), ContainerSecrets(), EnvFile(), EnvPairs(), Quadlet(), SecretPreflight() (+19 more)
...
Cohesion: 0.11
Nodes (27): TestAllowCommandIsRegisteredOnPlatformTrees(), TestAllowCommandIsRepeatable(), collectPaths(), findCmd(), TestExecute(), TestNotImplemented(), TestTreeStructure(), runComplete() (+19 more)
...
Cohesion: 0.24
Nodes (10): orNone(), orValue(), setOrMissing(), setOrNone(), GenOperator(), operatorImage(), RenderOperator(), TestOperatorImage() (+2 more)
...
### Community 70 - "quadletEscape"
Cohesion: 0.67
Nodes (3): escapePercent(), quadletEscape(), TestQuadletEscape()

## Knowledge Gaps
- **177 isolated node(s):** `solace`, `showCmd`, `Config`, `operatorTmplVars`, `Cluster` (+172 more)
...
- **Why does `Config` connect `Config` to `bg`, `cli_test.go`, `ctrCfg`, `Cluster`, `captureStdout`, `Manager`, `newTestOps`, `capRunner`, `NewCluster`, `convert_test.go`, `manager_test.go`, `Cluster`, `prep_test.go`, `verify_local_test.go`, `render.go`, `recRunner`, `Compose`, `haCfg`, `kubectlTransport`, `secrets_test.go`, `context.Context`, `.Run`, `hasCall`, `containerTransport`, `NewManager`, `RenderOperator`?**
  _High betweenness centrality (0.059) - this node is a cross-community bridge._
- **Why does `Platform` connect `Platform` to `ctrCfg`, `captureStdout`, `config_test.go`, `Manager`, `Config`, `capRunner`, `convert_test.go`, `manager_test.go`, `tierFor`, `github.com/spf13/cobra.Command`, `CheckCommand`, `manager.go`, `Compose`, `execguard_test.go`, `load.go`, `.Run`, `Load`, `containerTransport`, `NewManager`?**
