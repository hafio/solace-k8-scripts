...
echoUsage() {
  echo "Usage: $0
  (HA only) Exercise broker redundancy: release activity from the Primary, confirm the Backup
  takes over, then revert activity back to the Primary. Takes no positional arguments."
}
...
if [[ "${SOLBK_REDUNDANCY}" == "true" ]]; then
  ...
  ${KUBE} cp -n ${SOLBK_NS} ${TMPFILE} ${SOLBK_NAME}-pubsubplus-p-0:/usr/sw/jail/cliscripts/.show-rd.cli
  ${KUBE} cp -n ${SOLBK_NS} ${TMPFILE} ${SOLBK_NAME}-pubsubplus-b-0:/usr/sw/jail/cliscripts/.show-rd.cli
...
enable
configure
redundancy release-activity" > ${TMPFILE}
  ${KUBE} cp -n ${SOLBK_NS} ${TMPFILE} ${SOLBK_NAME}-pubsubplus-p-0:/usr/sw/jail/cliscripts/.release.cli
...
enable
configure
no redundancy release-activity" > ${TMPFILE}
  ${KUBE} cp -n ${SOLBK_NS} ${TMPFILE} ${SOLBK_NAME}-pubsubplus-p-0:/usr/sw/jail/cliscripts/.no-release.cli
