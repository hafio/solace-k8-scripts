...
echoUsage() {
  echo "Usage: $0
  Apply the product key(s) from \$SOLBK_PRODUCTKEYS to the Primary node (and Backup in HA).
  Output is captured in cli.out. Takes no positional arguments."
}
...
admin" > ${TMPFILE}
  ...
  if [[ $(grep -Ei 'error|fail' cli.out | wc -l) -ge 1 ]]; then
