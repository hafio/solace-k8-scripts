...
echoUsage() {
  echo "Usage: $0
  List all PubSub+ broker pods, services and statefulsets across every namespace (requires jq).
...
command -v jq >/dev/null 2>&1 || { echo "[Error] jq is required but not installed. Install jq and re-run."; exit 1; }
