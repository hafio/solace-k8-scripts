...
echoUsage() {
  echo "Usage: $0
  Report which deployment variables from the chosen env file are set (existence only, not
  correctness). Takes no positional arguments."
}
...
[[ -n "${SOLBK_ADM_PASS}" ]] && echo "[ OK  ] Solace Broker Admin Password specfied" || echo "[ERROR] No Solace Broker Admin Password"
