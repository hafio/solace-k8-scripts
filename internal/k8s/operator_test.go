...
func TestOperatorApply(t *testing.T) {
	...
	if len(rr.calls) != 2 {
		t.Fatalf("OperatorApply made %d calls, want 2 (regcred + bundle)", len(rr.calls))
	}
	reg, bundle := rr.calls[0], rr.calls[1]
...
func TestOperatorApplyNoPullSecret(t *testing.T) {
	...
	if len(rr.calls) != 1 {
		t.Fatalf("OperatorApply without pull secret made %d calls, want 1 (bundle only)", len(rr.calls))
	}
}
