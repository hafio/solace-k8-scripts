...
while [[ $(${KUBE} get pods -n ${SOLBK_NS} 2> /dev/null | grep "${SOLBK_NAME}-pubsubplus-p" | grep -c Running) -eq 0 ]]; do
  sleep 5
done
...
if [[ "${SOLBK_REDUNDANCY}" == "true" ]]; then
  ...
  while [[ $(${KUBE} get pods -n ${SOLBK_NS} 2> /dev/null | grep "${SOLBK_NAME}-pubsubplus-b" | grep -c Running) -eq 0 ]]; do
    sleep 5
  done
